...
def test_database_functionality(conn) -> bool:
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            print_success(f"Query active templates: {len(active_templates)} templates found")
            for template in active_templates:
                print(f"   - {template['name']} (v{template['version']}, {template['category']})")
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            print_success(f"Query active guardrails: {len(active_guardrails)} guardrails found")
            for guardrail in active_guardrails:
                print(f"   - {guardrail['name']} ({guardrail['severity']}, {guardrail['action']})")
...
def print_final_summary(results: Dict[str, Any]):
    ...
    if all_passed:
        ...
        print(f"{Colors.GREEN}{Colors.BOLD}✓ ALL VERIFICATION CHECKS PASSED{Colors.END}")
        print(f"{Colors.GREEN}{Colors.BOLD}  Enterprise Prompt System Migration: SUCCESS{Colors.END}")
        print(f"{Colors.GREEN}{Colors.BOLD}{'='*70}{Colors.END}")
        return 0
    else:

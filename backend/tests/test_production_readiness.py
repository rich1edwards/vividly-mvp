...
class TestWorkerIdempotency:
    ...
    def test_completed_request_allows_new_with_same_correlation(
        ...
        service.mark_completed(
            request_id=request1.id, video_url="https://example.com/video.mp4"
        )
        ...
        assert existing is None or existing.status in [
...
class TestMetricsExport:
    ...
    def test_metrics_failure_does_not_crash_worker(self, db_session: Session):
        ...
        with patch.dict("os.environ", {"GOOGLE_CLOUD_PROJECT": "test-project"}):
            ...
            with patch.object(
                worker, "export_metrics", side_effect=Exception("Metrics failed")
            ):

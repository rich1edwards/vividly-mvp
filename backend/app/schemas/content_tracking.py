...
class ProgressTrackingRequest(BaseModel):
    ...
    current_time_seconds: int = Field(
        ..., ge=0, description="Current playback position"
    )
    duration_seconds: int = Field(..., gt=0, description="Total content duration")
    playback_speed: float = Field(
        default=1.0, ge=0.25, le=2.0, description="Playback speed"
    )
...
class CompletionTrackingRequest(BaseModel):
    ...
    completion_percentage: float = Field(
        ..., ge=0, le=100, description="Percentage completed"
    )
    skipped_segments: List[dict] = Field(
        default=[], description="Segments that were skipped"
    )
...
class ContentAnalytics(BaseModel):
    ...
    avg_watch_duration: float = Field(
        ..., description="Average watch duration (seconds)"
    )
